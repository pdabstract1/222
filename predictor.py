...
with st.form("prediction_form"):
    ...
    # 呼吸频率：滑块输入
    RR = st.slider("呼吸频率 (次/分):", min_value=0, max_value=120, value=62, help="正常成人呼吸频率：12-20次/分")

    # 降钙素原：滑块输入，支持3位小数
    PCT = st.slider("降钙素原 (ng/mL):", min_value=0.000, max_value=10.000, value=1.750, step=0.001, format="%.3f", help="正常值：<0.05 ng/mL，建议输入3位有效数字")

    # 白细胞：滑块输入
    WBC = st.slider("白细胞计数 (×10⁹/L):", min_value=0.00, max_value=50.00, value=25.27, step=0.01, format="%.2f", help="正常范围：4.0-10.0 ×10⁹/L")
...
if submitted:
    ...
    if predicted_class == 1:
        probability = predicted_proba[1] * 100  # 患病的概率
        advice = (
            f"根据我们的模型，新生儿患有早发型败血症的风险较高。 "
            f"模型预测患有早发型败血症的概率为 {probability:.1f}%。 "
            "建议您咨询医疗保健提供者进行进一步评估和可能的干预。"
        )
    else:
        probability = predicted_proba[0] * 100  # 未患病的概率
        advice = (
            f"根据我们的模型，新生儿患有早发型败血症的风险较低。 "
            f"模型预测未患有早发型败血症的概率为 {probability:.1f}%。 "
...
if st.session_state.prediction_made:
    ...
    if st.session_state.predicted_class == 1:
        class_label = "患病 (1)"
        probability = st.session_state.predicted_proba[1] * 100  # 患病的概率
    else:
        class_label = "未患病 (0)"
        probability = st.session_state.predicted_proba[0] * 100  # 未患病的概率
        
    st.write(f"**预测类别:** {class_label}")
    st.write(f"**预测概率:** {probability:.1f}%")
