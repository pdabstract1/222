...
#     # 计算类别1的概率（患病概率）
#     probability_1 = predicted_proba[1] * 100

#     # 根据预测结果生成建议
#     if predicted_class == 1:
#         advice = (
#             f"根据我们的模型，您患有心脏病的风险较高。 "
#             f"模型预测您患有心脏病的概率为 **{probability_1:.2f}%**。 "
#             "建议您咨询医疗保健提供者进行进一步评估和可能的干预。"
#         )
#     else:
#         advice = (
...
#     probability_1 = st.session_state.predicted_proba[1] * 100
...
#     with col1:
#         if st.session_state.predicted_class == 1:
#             st.error(f"**预测结果: {class_label}**")
#         else:
#             st.success(f"**预测结果: {class_label}**")
    
#     with col2:
#         st.info(f"**患病概率: {probability_1:.2f}%**")
...
#     # 显示已保存的 SHAP 图
#     st.image("shap_force_plot.png", caption='SHAP 力解释图 - 显示各特征对预测的贡献')
